...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1>
            @lang('pegawai.title')
            <small>@lang('pegawai.title_edit')</small>
        </h1>
        <ol class="breadcrumb">
            <li><a href="{ route('dashboard') }}"><i class="fa fa-dashboard"></i> Home</a></li>
            <li><a href="{{ route('pegawai.index') }}">@lang('pegawai.title')</a></li>
            <li class="active">@lang('pegawai.title_edit')</li>
        </ol>
    </section>
                    ...
                    <form role="form" method="POST" action="{{ route('pegawai.update', $pegawai->id) }}">
                        ...
                        <div class="box-body">
                            ...
                            <div class="form-group {{ $errors->has('nip') ? 'has-error' : '' }}">
                                ...
                                <input type="text" class="form-control" id="nip" name="nip" placeholder="NIP" value="{{ $errors->any() ?  old('nip') : $pegawai->nip }}">
                                <div class="help-block">{{ $errors->first('nip') }}</div>
                            </div>

                            <div class="form-group {{ $errors->has('nama') ? 'has-error' : '' }}">
                                <label for="nama">@lang('pegawai.form_pegawai_nama_label')</label>
                                <input type="text" class="form-control" id="nama" name="nama" placeholder="nama" value="{{ $errors->any() ? old('nama') : $pegawai->nama }}">
                            ...
                            <div class="form-group {{ $errors->has('jabatan_id') ? 'has-error' : '' }}">
                                ...
                                <select name="jabatan_id" id="" class="form-control">
                                    ...
                                    @foreach($dataJabatan as $jabatan)
                                        <option value="{{ $jabatan->id }}" {{ $errors->any() ? (old('jabatan_id') == $jabatan->id ? 'selected' : '') : ($jabatan->id == $pegawai->jabatan_id ? 'selected' : '') }}>{{ $jabatan->nama }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group {{ $errors->has('tanggal_lahir') ? 'has-error' : '' }}">
                                ...
                                <input type="date" class="form-control" id="tanggal_lahir" name="tanggal_lahir" placeholder="tanggal_lahir" value="{{ $errors->any() ? old('tanggal_lahir') : $pegawai->tanggal_lahir->format('Y-m-d') }}">
                            ...
                            <div class="form-group {{ $errors->has('jenis_kelamin') ? 'has-error' : '' }}">
                                ...
                                <select name="jenis_kelamin" id="" class="form-control">
                                    ...
                                    <option value="0" {{ $errors->any() ? (old('jenis_kelamin') == '0' ? 'selected' : '') : ($pegawai->jenis_kelamin == '0' ? 'selected' : '') }}>Laki-laki</option>
                                    <option value="1" {{ $errors->any() ? (old('jenis_kelamin') == '1' ? 'selected' : '') : ($pegawai->jenis_kelamin == '1' ? 'selected' : '') }}>Perempuan</option>
                                </select>
                            ...
                            <div class="form-group {{ $errors->has('nomor_telepon') ? 'has-error' : '' }}">
                                ...
                                <input type="text" class="form-control" id="nomor_telepon" name="nomor_telepon" placeholder="Nomor Telepon" value="{{ $errors->any() ? old('nomor_telepon') : $pegawai->nomor_telepon }}">
                            ...
                            <div class="form-group {{ $errors->has('status_perkawinan') ? 'has-error' : '' }}">
                                ...
                                <select name="status_perkawinan" id="" class="form-control">
                                    ...
                                    <option value="0" {{ $errors->any() ? (old('status_perkawinan') == '0' ? 'selected' : '') : ($pegawai->status_perkawinan == '0' ? 'selected' : '') }}>Belum Kawin</option>
                                    <option value="1" {{ $errors->any() ? (old('status_perkawinan') == '1' ? 'selected' : '') : ($pegawai->status_perkawinan == '1' ? 'selected' : '') }}>Kawin</option>
                                </select>
                            ...
                            <div class="form-group {{ $errors->has('provinsi_id') ? 'has-error' : '' }}">
                                ...
                                <select name="provinsi_id" id="provinsi" class="form-control">
                                    ...
                                    @foreach($dataProvinsi as $provinsi)
                                        <option value="{{ $provinsi->id }}" {{ $errors->any() ? (old('provinsi_id') == $provinsi->id ? 'selected' : '') : ($provinsi->id == $pegawai->provinsi_id ? 'selected' : '')}}>{{ $provinsi->nama }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group {{ $errors->has('alamat') ? 'has-error' : '' }}">
                                ...
                                <textarea name="alamat" id="" cols="30" rows="10" class="form-control" placeholder="Alamat">{{ $errors->any() ? old('alamat') : $pegawai->alamat }}</textarea>
				...
				$.ajax({
					...
					success:function(data){
						...
						$.each(data,function(i,e){
							var kota_id = "{{ $errors->any() ? old('kota_id') : $pegawai->kota_id }}";
